...
**How to apply:** `.replit` must have `modules = ["nodejs-20", ...]`. If it shows nodejs-18, upgrade via `installProgrammingLanguage({ language: "nodejs-20" })` and reinstall all node_modules (root + frontend/) after.
