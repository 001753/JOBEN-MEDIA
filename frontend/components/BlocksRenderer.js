      ...
      blocks={{
        image: ({ image }) => (
          <figure className="my-6">
            <div className="relative w-full overflow-hidden rounded-xl bg-gray-100" style={{ aspectRatio: `${image.width}/${image.height}` }}>
              <Image
                src={image.url}
        ...
        link: ({ children, url }) => (
          <a href={url} className="text-brand-700 hover:text-brand-800 underline" target="_blank" rel="noopener noreferrer">
            {children}
          </a>
        ),
      }}
